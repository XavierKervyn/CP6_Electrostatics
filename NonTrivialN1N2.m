%% Paramètres: cas non trivial avec facteur /////////////////////////////
ViewFormat;
Ntotal  = round(logspace(1,3,nsimul));
...
% end
%     %extrapolation
%     P = polyfit(1./(Ntotal.^2), phirb,1); yintercept = P(2); 
%     err = abs((phirb - yintercept)/yintercept)*100;
%     
%     %plot 
%     plot(1./(Ntotal.^2), phirb,'x','Linewidth',lw,'HandleVisibility','off');
%     hold on
%     abscisse = 1./(Ntotal.^2); legendStrings = string(P(2)); xrange = (0:1.e-7:max(abscisse));
...
%     title(leg, '$\phi_{as}$ [V]')
%     xlabel("$1/N_{tot}^2$, $N_1 =$" + num2str(MeshFactor_) +"$N_2$"); ylabel('$\phi(r=b)$ [V]');
