...
figure('Name','plot e_r et rho_lib')
    r1      = 0:1.e-3:0.3; r2 = 0.3:1.e-3:0.5; r  = [r1 r2];
    eps_r   = [atan((r1/b_).^2).*ones(1,length(r1)) 1./(1+(r2/R_).^5).*ones(1,length(r2))];
    rho_lib = [a0_*exp(-(r1/b_).^2).*ones(1,length(r1)) a0_./log(r2/R_).*ones(1,length(r2))];
    hold on
    yyaxis left
    plot(r, eps_r,'Linewidth',lw);
    xlabel('$r$ [m]');
    ...
    hold off
    grid on; hold on; set(gca,'fontsize',fs);
SaveIMG("PlotEpsilonRrhoLibBONUS");
