...
fprintf(fid,['p=',num2str(p_loc),'\n']);% methode d'integration: p=1 trapeze p=0 midpoint

%écriture dans des fichiers:
fprintf(fid,['outputDataProfiles= N1_',num2str(N1_loc),'N2_',num2str(N2_loc),'p_',num2str(p_loc),'_dataprof.out\n']); % ficher output de  r_i epsilon_r(r_i) et rho_lib(r_i)/epsilon_0
fprintf(fid,['outputPotential = N1_',num2str(N1_loc),'N2_',num2str(N2_loc),'p_',num2str(p_loc),'_phi.out\n']); % ficher output de r_i et phi(r_i)
fprintf(fid,['outputElectricDisplacementFields = N1_',num2str(N1_loc),'N2_',num2str(N2_loc),'p_',num2str(p_loc),'_E_D.out\n']); %ficher d'output de r_mid,i, E_r(r_mid,i) et D_r(r_rmid,i)/epsilon_0
fprintf(fid,['outputDivergences = N1_',num2str(N1_loc),'N2_',num2str(N2_loc),'p_',num2str(p_loc),'_div_E_D.out\n']); % ficher output de r_midmid,i, div E(r_midmid,i) et div D(r_midmid,i)/epsilon_0 
